...
LangChain provides a native `FlashrankRerank` document compressor that can be used inside a `ContextualCompressionRetriever`. While using this wrapper would slightly reduce our lines of code, we explicitly chose a **custom implementation** for enterprise production readiness:

1.  **Granular Observability (Logfire)**: LangChain's wrapper abstracts the reranking process into a "black box." Our custom implementation allows us to inject precise tracking directly around the Cross-Encoder execution, logging the exact millisecond latency and top semantic scores.
2.  **Startup Performance (Lazy Loading)**: The native LangChain wrapper initializes the heavy ONNX model upon instantiation. This would severely delay the FastAPI server boot time and potentially cause "Logfire poisoning" if imported too early. Our system loads the model *lazily*, ensuring instant server startup.
