...
async def _cooldown(seconds: int, label: str, status_cb=None):
    ...
    for _ in range(seconds // 10): await asyncio.sleep(10)
    if status_cb: status_cb(f"✅ Ready — starting next experiment.")
...
async def _batched_score(metric, inputs: list, samples: list, status_cb=None, label: str = "") -> list:
    ...
    for b_idx, batch in enumerate(batches):
        if b_idx > 0: await _cooldown(COOLDOWN_MINI, f"{label} batch {b_idx}", status_cb)
...
async def run_all_metrics(golden_dataset: dict, status_cb=None) -> dict:
    ...
    if not samples: raise ValueError("No samples with actual_response found. Run Phase 1 first.")
    ...
    with logfire.span("🧪 Eval Phase 2 — All Metrics", total_samples=len(samples)):
        ...
        # Exp 1: Faithfulness
        if status_cb: status_cb(f"🧪 Exp 1/6 — Faithfulness ({len(samples)} samples)...")
        ...
        if status_cb: status_cb(f"🧪 Exp 2/6 — Answer Relevancy ({len(samples)} samples)...")
        ...
        if status_cb: status_cb(f"🧪 Exp 3/6 — Context Precision ({len(samples)} samples)...")
        ...
        if status_cb: status_cb(f"🧪 Exp 4/6 — Context Recall ({len(samples)} samples)...")
        ...
        if status_cb: status_cb(f"🧪 Exp 5/6 — Answer Correctness batch 1/2...")
        ...
        if status_cb: status_cb("⚡ Exp 6/6 — Tool Correctness (zero LLM calls)...")
        ...
        if status_cb: status_cb("✅ All 6 experiments complete!")
